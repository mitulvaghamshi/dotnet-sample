...
- Hover over the `json` variable and click the magnifying glass to see a text
  representation of the __JSON__.
- Close the dialog.
- Hover again, but click the drop-down to the right of the magnifying glass and
  select __JSON Visualizer__.
- Close the dialog.
- Stop the debugger and remove the breakpoint.
- Add the `using System.Text.Json;` directive, parse the __JSON__ into the list of
  `posts` and return them in the view.
...
public async Task<IActionResult> Index()
{
    try
    {
        ...
        using (var client = new HttpClient())
        {
            ...
            var response = await client
                .GetAsync("https://jsonplaceholder.typicode.org/posts");
...
- Set View name to `Index`, `Template` to `List`, `Model` class to `Post (Blog.Models)`,
  click __Add__, click __Yes__ to replace the existing view.
