...
- Update the `Index` method to use __LINQ__ to retrieve current admissions only (`DischargeDate` of `null`) and to sort by `Patient`s `LastName`, then `FirstName`.
...
- Update the `Index` method to accept a parameter for `NursingUnitId`, populate a
...
- Update `AdmissionExists` and the __POST__ `Edit` to use `admissionDate`.
