...
- Right-click the project in Solution Explorer and select `Manage NuGet
  Packages...`, notice that there is only one package installed.
...
- Intermittently, the scaffolder will fail and suggest installing
  `Microsoft.VisualStudio.Web.CodeGeneration.Design`. This package should already
  be installed. If this error occurs, select Rebuild Solution from the Build
  menu. If that doesn't succeed, close the solution and re-open it. If that
  still doesn't succeed, close Visual Studio and re-open. If that still doesn't
  solve it, uninstall and reinstall the package.
...
- Update the connection string to use __SQL Server Express__ and rename the database
  to `movie_tracker`.
...
- Save the file.
- Run the site. Click the `Movies` link, an error will occur because the database
  doesn't exist yet.
...
- Hover over `ModelState`, expand `ModelState / Values / Results View / [2]` to see
  new title.
- Step through the code and continue when reaching a return.
...
- In the `ConfigureServices` method, comment out the existing SQL Server database
  dependency injection and add an __SQLite__ injection with a different connection
  string.
...
- Add a connection string for __SQLite__
...
- Change the project back to SQL Server by commenting out the __SQLite__ injection
  in favour of SQL Server in `ConfigureServices`.
